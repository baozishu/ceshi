...
export function SiteProvider({ children }: { children: ReactNode }) {
    ...
    async function loadSettings() {
      try {
        ...
        if (settingsResponse.ok) {
          ...
          // 加载注册商图标
          const iconsResponse = await fetch("/api/registrar-icons")
          if (iconsResponse.ok) {
            const registrarIcons = await iconsResponse.json()
  ...
  const updateLogoText = async (text: string) => {
    try {
      const response = await fetch("/api/site-settings", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ logoText: text }),
      })
  ...
  const addRegistrarIcon = async (name: string, svg: string) => {
    try {
      ...
      const response = await fetch("/api/registrar-icons", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const removeRegistrarIcon = async (name: string) => {
    try {
      ...
      const response = await fetch(`/api/registrar-icons?name=${name}`, {
        method: "DELETE",
      })

      if (response.ok) {
        setSettings((prev) => {
          const updatedIcons = { ...prev.registrarIcons }
          delete updatedIcons[name]
          return {
            ...prev,
            registrarIcons: updatedIcons,
          }
        })
      }
    } catch (error) {
